...
class GalleryPagingSource(private val apiService: NasaApi) : PagingSource<Int, NasaResponse>() {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, NasaResponse> {
        ...
        val daysPerRequest = if (currentPage == 0) 14 else 15  // Number of days to fetch per request
        ...
        return try {
            ...
            if (response.isSuccessful && response.body() != null) {
                val photos = response.body()!!
                    .filter { it.mediaType != "video" } // Filter out video entries
                    .reversed() // Reverse the list of photos

                val nextKey = if (currentPage * daysPerRequest < totalDays) currentPage + 1 else null
